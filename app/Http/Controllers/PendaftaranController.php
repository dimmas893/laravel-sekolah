...
class PendaftaranController extends Controller
{
    ...
    public function pilihpendaftaran(Request $request)
    {
        //memilih pendaftaran
        $siswapindahan = $request->pendaftaran;
        if ($siswapindahan === 'siswapindahan') {
            return redirect()->route('siswapindahan');
        } else {
    ...
    {
       //mengambil data pendaftaran berdasarkan status 0 dan berdasarjan jenjang yg sudah dipilih
        $emps = Pendaftaran::where('jenjang', $id)->where('status', '==', '0')->get(); //mengambil data pendaftaran berdasarkan jengjang dan status 0
        $output = '';
        $p = 1;
        if ($emps->count() > 0) {
    ...
    {
        //post pendaftaran siswa baru
        $tahun = Setting::first()->id_tahun_ajaran; //mengambil data tahun ajaran yang berlangsung
        $tahun_ajaran = Tahun_ajaran::where('id', $tahun)->first()->tahun_ajaran; //mengambil data ajaran yang berlaku
        if ((int)$request->ceksekolah === 1) {
        ...
        $tanggalseleksi = TanggalSeleksi::where('status', 'aktif')->where('tahun_ajaran_id', $tahun)->first(); //engambil data tanggal seleksi berdasarkan yang aktif dan berdasarkan id tahun ajaran
        if (Pendaftaran::where('tingkat', $tingkat)->where('asal_sekolah', $asal)->where('nama_siswa', $request->nama_siswa)->where('jenjang', $request->ceksekolah)->where('tempat_lahir', $request->tempat_lahir)->where('tgl_lahir', $request->tgl_lahir)->where('jenis_kelamin', $request->jenis_kelamin)->where('agama', $request->agama)->where('no_telp', $request->no_telp)->where('email', $request->email)->where('no_telp_bapak', $request->no_telp_bapak)->where('no_telp_ibu', $request->no_telp_ibu)->first() === null) {
            ...
            $tanggalseleksi = TanggalSeleksi::where('status', 'aktif')->where('tahun_ajaran_id', $tahun)->first();
            ...
            try {
                ...
                $mail->SMTPSecure = 'tls';
                $mail->Port = 587;
                $mail->setFrom("dapurkod@dapurkoding.my.id", "Biaya Pendaftaran");
    ...
    public function update($id, Request $request)
    {
        ...
        $emp->update($create); //update data siswa
        return back();
    }
}
