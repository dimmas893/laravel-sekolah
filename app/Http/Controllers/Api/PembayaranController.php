...
class PembayaranController extends Controller
{
    ...
    {
        //detail pembayaran
        $pembayaran = Pembayaran::where('id_pembayaran', $request->id_pembayaran)->first(); //mengambil data pembayaran berdasarkan id pembayaran
    ...
    public function xenditAdi(Request $request)
    {
        ...
        $result = DB::transaction(function () use ($request) {
            ...
            $emp = Pembayaran::create($datapembayaran); //create pembayaran
            $invoiceNo = $number;
            ...
            $failurUrl = url('xendit/failure-payment/' . $invoiceNo);    //kalau gagal

            $data = array();

            $tampung_total_tagihan = array();
            foreach ($request->item_bayar as $key => $value) {
                $invoice = Invoice_Tagihan::where('id_invoice', $value['id_bayar'])->first(); //mengambil data tagihan berdasarkan id invoice
                $tagihan_siswa = Tagihan_Siswa::where('id', $invoice->id_tagihan)->first(); //mengambil data tagihan siswa berdasarkan id
                $kategori_tagihan = Kategori_Tagihan::where('id', $tagihan_siswa->id_kategori_tagihan)->first(); //mengambil data tagihan kategori berdasarkan id
                $data[] = array(
                    'name' => '$tagihan_siswa->deskripsi',
                    'quantity' => 1,
                    'price' => $value['nominal'],
                );
                array_push($tampung_total_tagihan, $value['nominal']); //tampung total nominal
                $rincianPembayaranTagihan = [
                    'id_pembayaran' => $emp->id_pembayaran,
                    'id_invoice' => $value['id_bayar'],
                    'tanggal_pembayaran' => $emp->tanggal_pembayaran,
                    'nominal_pembayaran' => $value['nominal'],
                    'metode_pembayaran' => 'xendit',
                ];

                Rincian_Pembayaran::create($rincianPembayaranTagihan); //create rincian pembayaran item
            }
            ...
            foreach ($request->biaya_lain as $p => $value) {
                ...
                Rincian_Pembayaran::create($rincianPembayaranLainnya); //create rincian pembayaran fee
                ...
                array_push($tampung_total_biayaLainnya, $value['nominal']); //total semua nominal
            }
    ...
    {
        //tidak dipakai, yang di pakai di web.php
        ...
        $dataRincianPembayaran = Rincian_Pembayaran::where('id_pembayaran', $idInvoice)->get();
        foreach ($dataRincianPembayaran as $value) {
            if ($value->id_invoice != 1) {
                $invoice = Invoice_Tagihan::where('id_invoice', $value->id_invoice)->first();
                if ($value->nominal_pembayaran >=  $invoice->nominal) {
                    $invoice->update([
                        'status' => 'paid',
                    ]);
                }
        ...
        return view('halamanPembayaranSukses');
    }
    ...
    public function xenditFailurePayment($idInvoice)
    {
        ...
        return view('halamanPembayaranGagal');
    }


    public function store(Request $request)
    {
        //tidak dipakai tapi jangan di hapus
        $tes = [];
        foreach ($request->item_bayar as $key => $value) {
    ...
    public function pembayaran_detail_satuan(Request $request)
    {
        ...
        $pembayaran = Pembayaran::where('siswa_id', $dataSiswa->id)->where('status', 'paid')->get();
        // dd($pembayaran);
        $data = [];
        foreach ($pembayaran as $p) {
            ...
            // dd($rincian_pembayaran->id_invoice);
