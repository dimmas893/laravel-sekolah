...
class MenuController extends Controller
{
    ...
    public function menukenaikankelas()
    {
        ...
        $sma = Kelas::where('id_tahun_ajaran', $tahun)->whereHas('kelas', function ($q) use ($jenjangsma) {
            $q->where('jenjang_pendidikan_id', $jenjangsma);
        })->select('jurusan', 'id_master_kelas')->groupBy('jurusan', 'id_master_kelas')->get();
        // dd($sma);
        $jenjangtk = 4;
        $tk = Kelas::where('id_tahun_ajaran', $tahun)->whereHas('kelas', function ($q) use ($jenjangtk) {
            $q->where('jenjang_pendidikan_id', $jenjangtk);
    ...
    public function manageTugasMataPelajarangurujadwal(Request $request)
    {
        ...
        // $tugas = Tugas::where('mata_pelajaran_id', $mata_pelajaran_id)->where('id_tahun_ajaran', $tahun)
        return view('menu.admin.manage.tugas.jadwal', compact('jadwal', 'tanggalhariini', 'tahun', 'jenjang_pendidikan_id', 'mata_pelajaran_id', 'guru_id'));
    }
    ...
    public function manageTugasMataPelajarangurujadwalbuattugas(Request $request)
    {
        ...
        $tugas = Tugas::where('mata_pelajaran_id', $request->mata_pelajaran_id)->where('tahun_ajaran_id', $tahun)->where('kelas_id', $kelasku)->count();
        // dd($tugas);
        return view('menu.admin.manage.tugas.buat', compact('jadwal', 'tugas'));
    }
