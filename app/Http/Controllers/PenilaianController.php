...
class PenilaianController extends Controller
{
    public function penilaian(Request $request)
    {
        ...
        if ($request->jadwal_id) { //kondisi jika request jadwal ada
            $jadwal = Jadwal::where('id', $request->jadwal_id)->first(); //mengambil data jadwal berdasaran id
            $dataNilaiSiswa = Penilaian::where('jadwal_id', $jadwal->id)->where('guru_id', $jadwal->guru_id)->get(); //mengambil data penilaian berdasarkan jadwal id dan berdasarkan guru id
        }

        if ($request->kelas_id && $request->mata_pelajaran_id && $request->guru_id) { //kondisi jika request kelas id, request mata pelajaran dan request guru ada
            $jadwal = Jadwal::where('kelas_id', $request->kelas_id)->where('mata_pelajaran_id', $request->mata_pelajaran_id)->where('guru_id', $request->guru_id)->first(); //mengambil data jadwal berdasarkan kelas id , mata pelajaran id dan berdasarkan guru id
            $dataNilaiSiswa = Penilaian::where('jadwal_id', $jadwal->id)->where('guru_id', $jadwal->guru_id)->get(); //mengambil data penilaian berdasarkan jadwal id
        }
        if ($jadwal) { //kondisi jika data jadwal ada
            $rincian_siswa = Siswa::where('kelas', $jadwal->kelas_id)->get(); //mengambil data siswa berdasarkan kelas
            $count = $rincian_siswa->count(); //menghitung jumlah siswa berdasarkan kelas
        }
        $arrayNilaiSiswa = [];
        if ($request->cek) {
            $cek = 1;
    ...
    {
        //proses memasukan nilai ke sistem
    ...
    {
        $setting = Setting::first(); //mengambil data setting
        $tahun = $setting->id_tahun_ajaran; //mengambil data tahun ajaran yang berlangsung
        ...
        })->select('mata_pelajaran_id')->groupBy('mata_pelajaran_id')->get(); //mengambil data jadwal berdasarkan jenjang pendidikan dan tahun ajaran yang berlangsung
        $tingkatan = 14;
    ...
    {
        //menampilkan nilai ajax
        $mata_pelajaran_id = $request->mata_pelajaran_id;
        $mata_pelajaran = Jadwal::whereIn('tingkatan_id', [14])->select('kelas_id', 'mata_pelajaran_id')->get(); //mengambil data jadwal berdasarkan tingkatan id 14
        $tk = Kelas::with('kelas')->whereIn('tingkatan_id', [14])->get(); //mengamil data kelas berdasarkan tingkatan id 14
        $p = 1;
        $output = '';
        if ($mata_pelajaran_id === null) { //kondisi jika data jadwal tidak ada
        ...
        if ($mata_pelajaran_id) { //kondisi jika data jadwal ada
            $output .= '<div class="row">
                            <div class="col-12">';
    ...
    {
        //lihat nilai berdasarkan mata pelajaran dan berdasarkan siswa ajax
        $siswa = Siswa::where('kelas', $kelas)->get(); //mengambil data siswa berdasarkan kelas
        $p = 1;
        $output = '';
        $output .= '';
        ...
        foreach ($siswa as $sis) {
            $cek = Penilaian::where('siswa_id', $sis->id)->where('mata_pelajaran_id', $mata_pelajaran)->first(); //mengambil data penilaian berdasarkan siswa id dan berdasarkan mata pelajaran id
            if ($cek) {
                $output .= '        <tr>
                                        <td>' . $p++ . '</td>
    ...
    {
        //menampilkan data nilai berdasarkan kelas dan mata pelajaran
    ...
    {
        //edit nilai berdasarkan mata pelajaran dan kelas
        $mata_pelajaran_id = $mata_pelajaran;
        $kelas_id = $kelas;
        $jadwal = Jadwal::where('mata_pelajaran_id', $mata_pelajaran_id)->where('kelas_id', $kelas_id)->get(); //mengambil data jadwal berdasarkan mata pelajaran dan berdasarkan kelas
