    ...
    @if (Auth::user())
        ...
        <ul class="navbar-nav navbar-right">
            <li class="dropdown"><a href="#" data-toggle="dropdown"
                    ...
                    @if (Auth::user()->role == 5)
                        @if ($gambar->avatar)
                            <img alt="" src="{{ asset('avatar/' . $gambar->avatar) }}"
                                class="rounded-circle mr-1">
                        @elseif(asset('avatar/' . $gambar->avatar) == false)
                            <img alt="" src="{{ asset('defaut3.jpg') }}" class="rounded-circle mr-1">
                        @else
                            <img alt="" src="{{ asset('defaut3.jpg') }}" class="rounded-circle mr-1">
                        @endif
                    @endif
                    @if (Auth::user()->role == 3)
                        @if ($gambarguru->avatar)
                            <img alt="" src="{{ asset('guru/' . $gambarguru->avatar) }}"
                                class="rounded-circle mr-1">
                    ...
                    @if (Auth::user()->role == 1)
                        @if ($admin->avatar != null)
                            <img alt="" src="{{ asset('avatar/' . $admin->avatar) }}"
                                class="rounded-circle mr-1">
